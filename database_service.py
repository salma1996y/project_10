...
def get_data():
   ...
   try:
       ...
       print (sort_order)
       count = requests.args.get("count")
       cursor.execute("SELECT name_id, name_date, label_id, name from data_labeling INNER JOIN data_input ON name_id = id ORDER BY name_date %s limit %s ;" , result_dict[sort_order] ,str(count))
       return cursor.fetchall()
       cursor.close()
       connection.close()
   except:
